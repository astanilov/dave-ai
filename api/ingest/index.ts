...
import { fromCodeFile, fromConfluencePage, fromJiraIssue/*, fromSlackMessage */ } from "./normalizers";
...
async function main() {
 console.log('🔍 Starting data ingestion...');
  ...
  // Jira
  try {
    const jiraIssues = await fetchJiraIssues({ jql: cfg.jiraJql, projectKey: cfg.jiraProjectKey, max: cfg.jiraMaxIssues });
    const jiraNorm = jiraIssues.map(fromJiraIssue);
    rows.push(...jiraNorm.flatMap(toRagChunks));
    console.log(`[Jira] fetched ${jiraIssues.length} issues → ${rows.length} total chunks so far`);
  ...
  // Confluence
  try {
    const pages = await fetchConfluenceContent({ spaceKey: cfg.confluenceSpaceKey });
    const confNorm = pages.map(fromConfluencePage);
    rows.push(...confNorm.flatMap(toRagChunks));
    console.log(`[Confluence] fetched ${pages.length} pages → ${rows.length} total chunks so far`);
  } catch (err: AxiosError | any) {
