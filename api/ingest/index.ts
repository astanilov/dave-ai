...
async function main() {
  ...
  try {
    ...
    rows.push(...(await Promise.all(jiraNorm.map(toRagChunks))).flat());
    console.log(
  ...
  try {
    ...
    rows.push(...(await Promise.all(confNorm.map(toRagChunks))).flat());
    console.log(
  ...
  //     rows.push(...(await Promise.all(slackNorm.map(toRagChunks))).flat());
