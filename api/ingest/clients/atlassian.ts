...
if (!cfg.atlassianBaseUrl || !cfg.atlassianEmail || !cfg.atlassianApiToken) {
  console.warn(
    '[WARN] Missing Atlassian credentials — Jira/Confluence calls will fail.'
  );
}
if (!cfg.slackToken) {
...
const atlassian = axios.create({
  baseURL: cfg.atlassianBaseUrl,
  timeout: 30000,
});

atlassian.interceptors.request.use(
  (config: InternalAxiosRequestConfig<any>) => {
    config.auth = {
      username: cfg.atlassianEmail as string,
      password: cfg.atlassianApiToken as string,
    };
    return config;
  }
);
