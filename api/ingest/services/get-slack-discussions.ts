...
async function fetchSlackDiscussions({ channelId, oldestTs = undefined, latestTs = undefined }: { channelId?: string; oldestTs?: string; latestTs?: string; }) {
  if (!channelId) return [];
  const messages = [];
  let cursor;
  do {
    const resp = await slack.conversations.history({ channel: channelId, limit: 1000, cursor, oldest: oldestTs, latest: latestTs, inclusive: false });
  ...
  const threaded = await Promise.all(messages
    .filter(m => m.thread_ts && m.reply_count && m.reply_count > 0)
    .map(m => limit(async () => {
      const replies = [];
      let cur;
      do {
        const r = await slack.conversations.replies({ channel: channelId, ts: m.thread_ts as string, limit: 1000, cursor: cur });
