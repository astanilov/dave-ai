...
function slackText(msg: any) {
  ...
  if (msg.blocks) {
    const bt = msg.blocks.map((b: any) => (
      b.elements ? b.elements.map((e: any) => e.elements ? e.elements.map((x: any) => x.text || '').join('') : '').join('') : ''
    )).join('\n');
    if (bt.trim()) t += '\n' + bt;
  }
  return t;
...
function isMarkdown(p: string) { return /\.(md|mdx)$/i.test(p); }

function isTestPath(p: string) {
  const s = p.toLowerCase();
  return (
    /\.(test|spec)\.[a-z0-9]+$/.test(s) ||
    s.includes('/__tests__/') || s.includes('\\__tests__\\') ||
    s.includes('/e2e/') || s.includes('\\e2e\\') ||
    s.includes('/integration/') || s.includes('\\integration\\') ||
    s.includes('/cypress/') || s.includes('\\cypress\\') ||
    s.includes('/playwright/') || s.includes('\\playwright\\')
  );
}
...
export { sleep, tsToISO, truncate, slackText, htmlToText, extractTextFromADF, personLite, cleanText, isMarkdown, isTestPath, markdownToText, includesLike };